...
public class ProductServiceImpl implements ProductService {
    ...
    //2、若商品详情图列表参数有值，对商品详情图进行相同的操作
    //3、将tb_product_img下面的该商品原先的商品详情图记录全部清除
    //4、更新tb_product信息
    public ProductExecution modifyProduct(Product product, ImageHolder thumbnail, List<ImageHolder> productImageHolderImgs) throws ProductOperationException {
        //空值判断
        if (product != null && product.getShop() != null && product.getShop().getShopId() != null) {
            //对商品设置上默认属性
            product.setLastEditTime(new Date());
            ...
            if (productImageHolderImgs != null && productImageHolderImgs.size() > 0) {
                deleteProductImgList(product.getProductId());
                addProductImgList(product, productImageHolderImgs);
            }
            try {
                //更新图片信息
